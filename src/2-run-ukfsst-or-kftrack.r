...
for(i in 1:5) {# seq_len(nrow(inits))) {
  ...
  save(fit, file = file.path(fit.dir, paste0(tagN,"_",kf.type,"_",i,".Rdata")))
  
  # ___ 2. 主要結果のCSVファイルの保存 ----------
  fit2csv(fit, folder = fit.dir, name = paste0(tagN,"_",kf.type,"_",i))
  # !!! この関数による一組の出力ファイル (例: fit2a.csv, fit2b.csv, fit2c.csv)
  # には, ファイル名に接尾がつきます. 各接尾の意味は
  # <出力ファイル名>-a.csv - 補正された経緯度等
  # <出力ファイル名>-b.csv - パラメータの初期値, 推定値等
  # <出力ファイル名>-c.csv - 尤度, Callしたモデル式など
  
  # ___ 3. モデル予測値と観測値の比較図の保存 -----------
  pdf(file = file.path(fig.dir, paste0(tagN,"_",kf.type,"_FitPlot_",i,".pdf")),
